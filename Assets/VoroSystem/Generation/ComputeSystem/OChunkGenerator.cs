...
public class OChunkGenerator : MonoBehaviour {
    ...
    void UpdateChunksToLoad() {
        ...
        var neighborChunkCoords = new Vector2[(int)Mathf.Pow(ChunkRenderDistance * 2, 2)];
        var i = 0;
        for (var z = (int)currentChunkCoord.y - ChunkRenderDistance;
             z < (int)currentChunkCoord.y + ChunkRenderDistance;
             z++) {
            for (var x = (int)currentChunkCoord.x - ChunkRenderDistance;
                 x < (int)currentChunkCoord.x + ChunkRenderDistance;
                 x++) {
        ...
        foreach (var cd in ChunkDataToLoad.ToArray()) {
        ...
        foreach (var chunkCoord in neighborChunkCoords) {
            if (Vector2.Distance(cameraPos_chunkSpace, chunkCoord + halfVec) < ChunkRenderDistance) {
                var index = ChunkDataToLoad.FindIndex(cd => cd.Coord == chunkCoord);
                if (index < 0) {
    ...
    void DeloadChunks() {
        foreach (var loadedCd in ChunkDataLoaded.ToArray()) {
            var index = ChunkDataToLoad.FindIndex(cd => cd.Coord == loadedCd.Coord);
            if (index < 0) {
                loadedCd.Deload();
    ...
    void LoadChunk(ChunkData cd) {
        cd.Init(chunkPrefab);
        Random.InitState(cd.RandomState);
        Chunk = cd.Chunk;
        Terrain = cd.Terrain;
        Water = cd.Water;
    ...
    void GenerateTerrainMaps() {
        ...
        for (var z = 0; z < ChunkSize + 2; z++) {
            for (var x = 0; x < ChunkSize + 2; x++) {
                var e = Mathf.PerlinNoise((x + xOffset - Seed + .01f) / ElevationMapScale,
                    (z + zOffset - Seed + .01f) / ElevationMapScale);

                // [-.292893219, .224744871]
                elevationValue = elevationValueSmooth = Mathf.Pow(e + .5f, .5f) - 1f;
                //Debug.Log(elevationValue);
                rough =
                    Mathf.Pow(Mathf.PerlinNoise((x + xOffset + .01f) / 30f, (z + zOffset + .01f) / 30f) + .5f,
                        .1f) - 1f;
                ...
                // MountainMap [0, 1]
                mountainValue = Mathf.PerlinNoise((x + xOffset - Seed + .01f) / ElevationMapScale,
                    (z + zOffset - Seed + .01f) / ElevationMapScale);
                ...
                rough =
                    Mathf.Pow(Mathf.PerlinNoise((x + xOffset + .01f) / 50f, (z + zOffset + .01f) / 50f) + .5f,
                        .1f) - 1f;
                temperatureValue += rough;

                float latitudeMod;
                latitudeMod = (Mathf.PerlinNoise((x + xOffset + .01f) / TemperatureMapScale,
                    (z + zOffset + .01f) / TemperatureMapScale) + .5f - 1f) * 1.5f;
                ...
                // HumidityMap [0, 1]
                humidityValue = Mathf.PerlinNoise((x + xOffset - Seed + .01f) / HumidityMapScale,
                    (z + zOffset - Seed + .01f) / HumidityMapScale);
                ...
                var riverWindingScale = 75f + 25f * (mountainValue * 2f - 1f);
                freshWaterValue = Mathf.PerlinNoise((x + xOffset - Seed + .01f) / riverWindingScale,
                    (z + zOffset - Seed + .01f) / riverWindingScale) * 2f - 1f;
                rough =
                    Mathf.Pow(Mathf.PerlinNoise((x + xOffset + .01f) / 10f, (z + zOffset + .01f) / 10f) + .5f,
                        .3f) - 1f;
                ...
                */
                ...
                var fwThreshhold = 1f +
                    Mathf.Pow(Mathf.PerlinNoise((x + xOffset + .01f) / 10f, (z + zOffset + .01f) / 10f) + .5f, 1f) - 1f;
                ...
                var mtnMod = Mathf.Pow(mountainValue + .5f, 1f) - 1f;
                wetnessValue += mtnMod;
                ...
                // BiomeMap
                biomeValue = OBiome.GetBiome(temperatureValue, humidityValue);
                ...
                for (var i = 0; i < octaves; i++) {
                    var sampleX = (x + xOffset) / scale * frequency + Seed;
                    var sampleZ = (z + zOffset) / scale * frequency + Seed;

                    var scale2 = scale + 5f;
                    var scale3 = scale - 5f;

                    var perlinValue = Mathf.PerlinNoise(sampleX, sampleZ) * 2 - 1;
                ...
                var heightCutoff = Mathf.Lerp(flatLevel + .01f, 1f, tMod);
                ...
                if (heightValue < flatLevel) {
                    ...
                    var f = Mathf.InverseLerp(0f, .004f, flatLevel - heightValue);
                ...
                if (heightValue >= seaLevel) {
                    ...
                    if (freshWaterValue > 0f) {
                        if (freshWaterValue > .93f) {
                            var f = Mathf.InverseLerp(.93f, 1f, freshWaterValue);
                            ...
                            if (freshWaterValue <= .9f) {
                                rough =
                                    (Mathf.PerlinNoise((x + xOffset) / .1f + .01f, (z + zOffset) / .1f + .01f) * 2f -
                                     1f) / 1f;
                            }
                ...
                if (biomeValue == (int)OBiome.BiomeType.Desert) {
                    var duneMag = .012f * (1f - Mathf.Pow(Mathf.Clamp01(freshWaterValue), 1.3f)) *
                                  (1f - Mathf.Pow(wetnessValue, 1.2f));
                    heightValue += duneMag * (1f -
                                              Mathf.Abs(Mathf.Sin((x + xOffset - Seed + .01f +
                                                                   Mathf.Sin(z + zOffset) * 8f) / 15f)));
                }
                else {
                    heightValue += .0025f * Mathf.PerlinNoise((x + xOffset - Seed + .01f) / 2f,
                        (z + zOffset - Seed + .01f) / 2f);
                }
    ...
    Color SetVertexColor(int biome, float height, float temperature, float humidity, float wetness, float fw) {
        ...
        if (biome == (int)OBiome.BiomeType.Tundra || biome == (int)OBiome.BiomeType.SnowyTaiga) {
            c.b = 255f;
            return c;
        }

        if (temperature < 5f / 11f) {
            if (height > snowLevel + (1f - temperature) * .05f * (Random.value * .05f)) {
                c.b = 255f;
                return c;
            }
        }
        ...
        if (biome == (int)OBiome.BiomeType.Desert) {
            c.g = 255f * Mathf.Clamp01(Mathf.Pow(fw, 1.5f)) * .3f;
        }
        else if (biome == (int)OBiome.BiomeType.Chaparral) {
            c.g = 255f * Mathf.Clamp01(Mathf.Pow(fw, 1.5f)) * .3f;
        }
        else {
            c.g = 255f * wetness;
    ...
    void PlaceTerrainAndWater() {
        ...
        for (int i = 0, z = 0; z < ChunkSize + 2; z++) {
            for (var x = 0; x < ChunkSize + 2; x++) {
                ...
                TerrainColors[i] = SetVertexColor(BiomeMap[x, z], HeightMap[x, z], TemperatureMap[x, z],
                    HumidityMap[x, z], WetnessMap[x, z], FreshWaterMap[x, z]);
        ...
        for (var z = 0; z < ChunkSize + 2; z++) {
            for (var x = 0; x < ChunkSize + 2; x++) {
                ...
                if (TreeMap[x, z]) {
                    ...
                    // tree placement
                    var treeTuple = OBiome.GetTree(biome, wetness, fw);
                    if (treeTuple != null && treeTuple.Item2.Item2 > 0f) {
                        PlaceTreeBundle(treeTuple, wetness, onWater, x, z);
                    }

                    // feature placement
                    var featureTuple = OBiome.GetFeature(biome, wetness, fw, onWater);
    ...
    void PlaceTreeBundle(Tuple<GameObject, Tuple<float, float, float, float, float>> treeTuple, float wetness,
        bool onWater, int x, int z) {
        var tree = treeTuple.Item1;
        var treeScale = treeTuple.Item2.Item1;
        var passesMultipler = (int)(treeTuple.Item2.Item2 * 10f);
        var treeMinYNormal = treeTuple.Item2.Item3;
        var treeAngleMultiplier = treeTuple.Item2.Item4;
        var spreadMultiplier = treeTuple.Item2.Item5;
        Quaternion uprightRot;
        Quaternion slantedRot;
        Vector3 castVec;
        float castLength;

        var passes = (int)(wetness * passesMultipler * treeDensity);
        passes = Mathf.Clamp(passes, (int)(Random.value + .25f), passes);
        if (passes > 0) {
            for (var j = 0; j < treeDensity * passes; j++) {
                castVec = new Vector3(x + xOffset + (Random.value * 2f - 1f) * spreadMultiplier * 10,
                    ElevationAmplitude, z + zOffset + (Random.value * 2f - 1f) * spreadMultiplier * 10);

                if (onWater) {
                    castLength = ElevationAmplitude - (seaLevel - .0003f) * ElevationAmplitude;
                }
                else {
                    castLength = ElevationAmplitude - (seaLevel + .002f) * ElevationAmplitude;
                }

                if (Physics.Raycast(castVec, Vector3.down, out var hit, castLength)) {
                    var point = hit.point;
                    if (hit.normal.y > treeMinYNormal) {
                        if (onWater) {
                            if (point.y < (seaLevel - .0018f) * ElevationAmplitude) {
                                uprightRot = Quaternion.AngleAxis(Random.value * 360f, Vector3.up);
                                slantedRot = Quaternion.FromToRotation(transform.up, hit.normal);

                                tree = Instantiate(tree, point,
                                    Quaternion.Slerp(uprightRot, slantedRot, treeAngleMultiplier), Trees.transform);
                                tree.transform.localScale =
                                    Vector3.one * treeScale * Mathf.Pow(Random.value + .5f, .2f);
                            }
                        }
                        ...
                        tree = Instantiate(tree, point, Quaternion.Slerp(uprightRot, slantedRot, treeAngleMultiplier),
                            Trees.transform);
                        tree.transform.localScale = Vector3.one * treeScale * Mathf.Pow(Random.value + .5f, .2f);
                    }
                }
            }
        }
    ...
    public static ChunkData GetChunk(Vector2 chunkCoord) {
        ...
        foreach (var cd in ChunkDataLoaded.ToArray()) {
    ...
    public static ChunkData GetChunk(Vector3 position) {
        var position_chunkSpace = ToChunkSpace(position);
        var chunkCoord = new Vector2((int)position_chunkSpace.x, (int)position_chunkSpace.y);
    ...
    public float Posterize(float value, float maxValue, int sections) {
        ...
        var sectionSize = maxValue / sections;
