...
/// </summary>
[UxmlElement]
public partial class LayerElement : VisualElement {
    public static LayerElement ActiveLayer;
    ...
    public static event Action<LayerElement> OnDiagramSelectedEvent;
    ...
    void Select() {
        if (ActiveLayer != null && ActiveLayer != this) {
            ActiveLayer.Active = false;
            ActiveLayer.RemoveFromClassList("panel-selected");
            var previousToggle = ActiveLayer.Q<Toggle>(); // todo replace Toggle with the selection button
        ...
        Debug.Log($"LayerElement {ActiveLayer.DisplayName} was selected");
