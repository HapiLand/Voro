using UnityEngine;
using Voro.VoroSystem.Template.Internal;

namespace Voro.VoroSystem.Template {
class VoroSystemManager : SystemLifecycle {
    readonly Transform _root;
    Core.VoroSystem.VoroSystem _system;

    public VoroSystemManager(Transform root) {
        _root = root;
    }

    /// <summary>
    /// Create a bounding region for the World Map, get configuration.json
    /// </summary>
    protected override void Initialize() {
        Debug.Log("[Voro System] Initialize");
        Debug.Log("[Voro System] Creating World Environment");
        _system = new Core.VoroSystem.VoroSystem();
